...
function encodeToRot13(str) {
  const alf = 'ABCDEFGHIJKLMabcdefghijklm';
  const rot13 = 'NOPQRSTUVWXYZnopqrstuvwxyz';
  let res = '';

  for (let i = 0; i < str.length; i += 1) {
    if (alf.includes(str[i])) {
      res += rot13[alf.indexOf(str[i])];
    } else if (rot13.includes(str[i])) {
      res += alf[rot13.indexOf(str[i])];
    } else {
      res += str[i];
    }
  }
